...
def test_alter():
    ...
    assert np.isclose(
        ds["Fc"].loc[795613, "FcgRIIIa.V158", "gp120.TRO"], 13146.1 + 681.9
    )
...
def test_jones():
    from ..jones import make_RA_Tensor, process_RA_Tensor
...
def test_kaplonekVaccineSA():
    ...
    assert df.loc[0, "FcR3B_WT.RBD"] == ds["Luminex"].sel(
        Subject=0, Antigen="WT.RBD", Receptor="FcR3B"
    )
